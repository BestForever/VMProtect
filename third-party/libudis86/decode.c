        ...
        if ( 0x66 == u->pfx_insn ) {
            ...
            if ((curr == 0x38) || (curr == 0x3A)) {
              ...
              if ( u->error )
                return -1;
              u->pfx_opr = 0;
            } else if ( ud_itab_list[ ITAB__PFX_SSE66__0F ][ curr ].mnemonic != UD_Inone ) {
            ...
            if (curr == 0x38) {
              ...
              if ( u->error )
                return -1;
              u->pfx_repne = 0;
            } else if (ud_itab_list[ITAB__PFX_SSEF2__0F][curr].mnemonic != UD_Inone) {
                table = ITAB__PFX_SSEF2__0F; 
                u->pfx_repne = 0;
            }
        } else if ( 0xF3 == u->pfx_insn ) {
            if ( ud_itab_list[ ITAB__PFX_SSEF3__0F ][ curr ].mnemonic != UD_Inone ) {
         ...
         unsigned char reg_size, unsigned char reg_type)
{
  ...
  /* else its memory addressing */  
	...
	/* no memory addressing with CR or DR registers */
	if (reg_type == T_CRG || rm_type == T_CRG ||
		reg_type == T_DBG || rm_type == T_DBG) {
		u->error = 1;
		return;
	}
    op->type = UD_OP_MEM;
